...
export function UserProfileView({ onSignOut }: UserProfileViewProps) {
  ...
  const getSpotlightBoostProduct = () => {
    return stripeProducts.find(product => product.id === 'prod_Sxkxm8chH8YAIF')
  }
  ...
  const spotlightBoost = getSpotlightBoostProduct()
  return (
    <div className="min-h-screen bg-gradient-to-br from-neutral-900 via-neutral-800 to-neutral-900 py-8">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-white">
                    {currentPlan ? currentPlan.name : 'Active Subscription'}
                  </h3>
                  <p className="text-gray-300 text-sm">
                    Status: <span className="capitalize text-[#FFC107]">{subscriptionData.subscription_status}</span>
              ...
              {currentPlan && (
                <div className="text-right">
                  ...
                  <p className="text-gray-400 text-sm">
                    {currentPlan.mode === 'subscription' ? 'per month' : 'one-time'}
                  </p>
                </div>
              )}
            </div>
        ...
        {/* Upgrade Section */}
        {(!subscriptionData || subscriptionData.subscription_status !== 'active') && (
          <div className="bg-gradient-to-br from-[#FFC107]/10 to-[#FFB300]/5 backdrop-blur-sm rounded-2xl p-8 border border-[#FFC107]/20 mb-8">
            <div className="text-center mb-6">
              <h2 className="text-2xl font-bold text-white mb-2 font-poppins">
                Upgrade to Pro
              </h2>
              <p className="text-gray-300">
                Get advanced features to accelerate your job search
              </p>
            </div>
            
            {stripeProducts.find(p => p.id === 'prod_SxkwP250qwXKUS') && (
              <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
                <div className="flex-1">
                  <div className="flex items-center space-x-4 mb-4">
                    <div className="text-3xl font-bold text-white">
                      {formatPrice(stripeProducts.find(p => p.id === 'prod_SxkwP250qwXKUS')!.price)}
                    </div>
                  ...
                  <ul className="space-y-2 text-sm text-gray-300">
                    <li className="flex items-center">
                      ...
                      Profile boost (higher in searches)
                    </li>
                    <li className="flex items-center">
                      <Check className="w-4 h-4 text-green-400 mr-2" />
                ...
                <StripeCheckout
                  product={stripeProducts.find(p => p.id === 'prod_SxkwP250qwXKUS')!}
                  onError={handleCheckoutError}
                  disabled={false} // UserProfileView is only shown to authenticated users
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
          ...
          {/* Spotlight Boost */}
          {spotlightBoost && (
            <div className="bg-gradient-to-br from-white/10 to-white/5 hover:from-purple-600/20 hover:to-purple-700/10 backdrop-blur-sm rounded-2xl p-6 border border-white/20 hover:border-purple-500/50 transition-all duration-300 hover:shadow-xl hover:shadow-purple-600/10 hover:-translate-y-1 group">
              <div className="flex items-center space-x-4 mb-4">
                <div className="w-14 h-14 bg-gradient-to-br from-purple-600 to-purple-700 rounded-2xl flex items-center justify-center group-hover:scale-110 group-hover:rotate-3 transition-all duration-300 shadow-lg shadow-purple-600/30">
                  <Star className="w-6 h-6 text-white" />
                </div>
                <div className="text-left flex-1">
                  <h3 className="text-white font-semibold text-lg group-hover:text-purple-400 transition-colors duration-300">Spotlight Boost</h3>
                  <p className="text-gray-400 text-sm group-hover:text-gray-300 transition-colors duration-300">48h enhanced visibility</p>
                  <p className="text-purple-400 font-medium text-sm">{formatPrice(spotlightBoost.price)}</p>
                </div>
              </div>
              <StripeCheckout
                product={spotlightBoost}
                onError={handleCheckoutError}
                disabled={false} // UserProfileView is only shown to authenticated users
                className="w-full bg-purple-600 hover:bg-purple-700 text-white py-2 rounded-lg font-medium transition-all duration-200"
              >
                Boost Profile
              </StripeCheckout>
            </div>
