...
export function AddOnsModal({ isOpen, onClose }: AddOnsModalProps) {
  ...
  // Get add-ons by category from Stripe config
  const getAddOnsByCategory = (category: 'promotion' | 'outreach' | 'enterprise') => {
    return getProductsByCategory(category).filter(product => 
      product.userType === 'employer' || product.userType === 'both'
    )
  }
  ...
  const filteredAddOns = getAddOnsByCategory(selectedCategory)

  const getAddOnCardStyle = (addon: any) => {
    if (addon.category === 'enterprise') {
  ...
  const getIcon = (addon: any) => {
    if (addon.name.includes('Silver')) return <Star className="w-6 h-6" />
    if (addon.name.includes('Gold')) return <Crown className="w-6 h-6" />
    if (addon.name.includes('Diamond')) return <Diamond className="w-6 h-6" />
    if (addon.name.includes('Platinum')) return <Sparkles className="w-6 h-6" />
    if (addon.name.includes('Annual')) return <Target className="w-6 h-6" />
    if (addon.name.includes('InMails')) return <Mail className="w-6 h-6" />
    if (addon.name.includes('Unlimited')) return <Users className="w-6 h-6" />
    if (addon.name.includes('Leaderboard')) return <Trophy className="w-6 h-6" />
    if (addon.name.includes('Enterprise')) return <Shield className="w-6 h-6" />
    return <Zap className="w-6 h-6" />
  }

  const getFeatures = (addon: any) => {
    // Define features based on product name/category
    if (addon.name.includes('Silver')) {
      return [
    ...
    if (addon.name.includes('Diamond')) {
      return [
        'All Gold features',
    ...
    if (addon.name.includes('Platinum')) {
      return [
    ...
    if (addon.name.includes('InMails 200')) {
      return [
    ...
    if (addon.name.includes('InMails 500')) {
      return [
    ...
    if (addon.name.includes('Unlimited')) {
      return [
    ...
    if (addon.name.includes('Leaderboard')) {
      return [
    ...
    if (addon.name.includes('Enterprise')) {
      return [
            ...
            >
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  {getFeatures(addon).map((feature, index) => (
                ...
                {/* Action Button */}
                {addon.category === 'enterprise' ? (
                  <Button
                    className="w-full py-3 rounded-lg font-semibold transition-all duration-200 bg-gradient-to-r from-purple-600 to-purple-700 hover:from-purple-700 hover:to-purple-800 text-white"
                  >
                    Contact Sales
                  </Button>
                ) : (
                  <StripeCheckout
                    ...
                    disabled={false} // AddOnsModal is only shown to authenticated users
