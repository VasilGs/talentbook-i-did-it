...
import { Building2, Upload, Globe, Phone, MapPin, User, FileText, Camera, Hash, Loader2, CheckCircle } from 'lucide-react'
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.companyLogo) {
        const uploadedUrl = await uploadFile(formData.companyLogo, 'company-logos')
        if (uploadedUrl) {
          companyLogoUrl = uploadedUrl.publicUrl
        } else {
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="flex justify-center">
            ...
            >
              {loading ? 'Creating Company Profile...' : !isVerified ? 'Verify Account to Continue' : 'Complete Company Profile'}
            </Button>
          </div>
        </form>
      </div>
