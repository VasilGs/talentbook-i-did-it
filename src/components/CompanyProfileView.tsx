...
import { stripeProducts, getProductsByUserType, formatPrice } from '../stripe-config'
...
export function CompanyProfileView({ company, onUpdateSuccess, onSignOut }: CompanyProfileViewProps) {
  ...
  const getEmployerPlans = () => {
    return getProductsByUserType('employer').filter(product => 
      product.category === 'subscription' && 
      (product.name.includes('Starter') || product.name.includes('Growth') || product.name.includes('Scale'))
    )
  }
            ...
            <div className="flex-1 min-w-0">
              <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-4">
                <div>
                  ...
                  {(company.subscription_package || currentPlan) && (
                    <div className="flex items-center mb-3">
                      <div className={`bg-gradient-to-r ${getSubscriptionColor(currentPlan?.name || company.subscription_package)} px-3 py-1 rounded-full flex items-center space-x-2`}>
                        <Crown className="w-4 h-4 text-white" />
                        <span className="text-white text-sm font-medium">
                          {currentPlan ? currentPlan.name : getSubscriptionLabel(company.subscription_package)} Plan
                        </span>
                      </div>
                    </div>
                  )}
          ...
          <div className="bg-gradient-to-br from-[#FFC107]/10 to-[#FFB300]/5 backdrop-blur-sm rounded-2xl p-8 border border-[#FFC107]/20 mb-8">
            <div className="text-center mb-6">
              ...
              <p className="text-gray-300">
                Unlock powerful recruiting features with our employer plans
              </p>
            </div>
            
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {employerPlans.map((plan) => (
                <div key={plan.id} className={`bg-white/5 rounded-xl p-6 border border-white/10 ${plan.popular ? 'border-[#FFC107]/40' : ''}`}>
                  {plan.popular && (
                    <div className="bg-[#FFC107] text-black px-3 py-1 rounded-full text-xs font-bold mb-4 inline-block">
                      POPULAR
                    </div>
                  )}
                  <h3 className="text-lg font-semibold text-white mb-2">
                    {plan.name.replace('Employer ', '')}
                  </h3>
                  ...
                  <StripeCheckout
                    ...
                    disabled={false} // CompanyProfileView is only shown to authenticated users
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
          ...
          <button 
            onClick={() => {
              ...
              const upgradeSection = document.querySelector('[data-upgrade-section]')
              if (upgradeSection) {
                upgradeSection.scrollIntoView({ behavior: 'smooth' })
              }
            }}
            className="bg-gradient-to-br from-white/10 to-white/5 hover:from-orange-600/20 hover:to-orange-700/10 backdrop-blur-sm rounded-2xl p-6 border border-white/20 hover:border-orange-500/50 transition-all duration-300 hover:shadow-xl hover:shadow-orange-600/10 hover:-translate-y-1 group"
          >
            <div className="flex items-center space-x-4">
