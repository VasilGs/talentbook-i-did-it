...
export function Pricing({
  ...
  const getEmployerFeatures = (planName: string) => {
  ...
  // Get plans from Stripe config
  const plans = useMemo(() => {
    if (selectedUserType === "job_seeker") {
      // Free plan + Pro subscription
      const proSubscription = stripeProducts.find(p => p.id === 'prod_SxkwP250qwXKUS')
      return [
        {
          key: "cand-basic",
          name: "Basic",
        ...
        ...(proSubscription ? [{
          key: proSubscription.id,
          name: "Pro",
          popular: true,
          billing: "monthly" as const,
          monthlyPrice: proSubscription.price / 100,
          description: proSubscription.description,
          features: [
          ...
          priceIdMonthly: proSubscription.priceId,
        }] : [])
      ]
    } else {
      // Employer plans from Stripe config
      const employerSubscriptions = getProductsByUserType('employer').filter(p => 
        p.category === 'subscription' && 
        (p.name.includes('Starter') || p.name.includes('Growth') || p.name.includes('Scale'))
      )
      
      return employerSubscriptions.map(product => ({
        key: product.id,
        name: product.name.replace('Employer ', ''),
        popular: product.popular || false,
        billing: "monthly" as const,
        monthlyPrice: product.price / 100,
        description: product.description,
        features: getEmployerFeatures(product.name),
        priceIdMonthly: product.priceId,
      }))
    }
  }, [selectedUserType])
  ...
  const canStripeCheckout = (plan: any) => {
    if (plan.billing === "both") {
  ...
  const stripePayloadFor = (plan: any) => {
    ...
    return {
      ...
      mode: (plan.billing === "free" ? "payment" : "subscription") as const,
      price: cents,
            ...
            >
              ...
              <div className="flex-1 flex flex-col">
                ...
                {canStripeCheckout(plan) ? (
                  ...
                  >
                    {plan.billing === "free" ? "Get Started Free" : "Get Started"}
                  </Button>
                )}
